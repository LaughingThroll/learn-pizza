...
const CartBody = () => {
  ...
  const { totalPrice, totalCount, cart, payPizzas, clearCart } = CartStore
  ...
  return (
    <div className="cart">
      <div className="cart__top">
        <h2 className="cart__title content__title">
      ...
      <div className="content__items">
        {oneOfPizzas.map(item => {
          return <CartItem key={`${item.id}_${item.type}_${item.size}`} {...item} />
        })}

      </div>
      <div className="cart__bottom cart-bottom">
        <div className="cart-bottom__details cart-details">
          <span className="cart-details__totaly-count"> Всего пицц: <b>{totalCount} шт.</b> </span>
          <span className="cart-details__totaly-price"> Сумма заказа: <b>{totalPrice} ₽</b> </span>
        </div>
        <div className="cart-bottom__buttons cart-buttons">
          <Link to="/" className="cart-buttons__go-back button button--outline button--add button-go-back">
            <svg width="8" height="14" viewBox="0 0 8 14" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M7 13L1 6.93015L6.86175 1" stroke="#D3D3D3" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
            <span>Вернуться назад</span>
          </Link>
          <Button className="cart-buttons__pay pay-btn" onClick={payPizzas}>
            Оплатить сейчас
          </Button>
        </div>
      </div>
...
export default observer(CartBody)
